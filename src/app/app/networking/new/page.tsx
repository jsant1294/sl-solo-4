...
export default async function NewConnection({ searchParams }: { searchParams: Promise<{ lang?: string }> }) {
  ...
  if (!access.settings.manualConnectionsEnabled) notFound();
  ...
  return (
    <div className="max-w-xl">
      ...
      <h1 className="font-display text-3xl font-semibold mt-4 mb-6">{es ? "Agregar contacto" : "Add Connection"}</h1>
      <ConnectionForm mode="manual" locale={locale} />
    </div>
  );
}
